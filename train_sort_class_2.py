...
def class_prune_setup(opt, model_path, iteration, views, gaussians, pipeline, background, classifier, selected_obj_ids, cameras_extent, removal_thresh):
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, start_pointcloud,load_iteration,debug_from, use_wandb):
    first_iter = 0
    opt.iterations = 3
    ...
    if checkpoint:
        ...
        gaussians = GaussianModel(dataset.sh_degree)
        scene = Scene(dataset, gaussians)
        gaussians.load_ply(os.path.join(start_pointcloud,"point_cloud","iteration_"+str(args.loaded_iteration),"point_cloud.ply"))
        ...
        classifier = torch.nn.Conv2d(gaussians.num_objects, num_classes, kernel_size=1)
        classifier.cuda()
        classifier.load_state_dict(torch.load(os.path.join(start_pointcloud,"point_cloud","iteration_"+str(args.loaded_iteration),"classifier.pth")))
    else:
        gaussians = GaussianModel(dataset.sh_degree)
        scene = Scene(dataset, gaussians)
        gaussians.training_setup(opt)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        ic(objects.shape())
        ic(gaussians._objects_dc.permute(2,0,1).shape())
        ...
        gaussians._objects_dc.grad[: ] = 0.0
        ...
        with torch.no_grad():
            ...
            if iteration in args.prune_iterations:
                ...
                #     )
                if args.prune_type == "opacity":
                    gaussians.sort_gaussians_class(
                        gaussians.get_opacity.detach()
                    )
                elif args.prune_type == "opacity_scale":
                    # v_list = imp_list * torch.max(gaussians.get_scaling, dim=1)[0]
                    ic(gaussians.get_scaling)
                    ic(torch.max(gaussians.get_scaling, dim=1)[0])
                    ic(gaussians.get_opacity.detach())
                    ic(gaussians.get_opacity.detach()[:,0])
                    gaussians.sort_gaussians_class(
                        gaussians.get_opacity.detach()[:,0]*torch.max(gaussians.get_scaling, dim=1)[0]
                    )
            # Optimizer step
            if iteration < opt.iterations:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--loaded_iteration", default=60000, type=int)
    parser.add_argument('--detect_anomaly', action='store_true', default=False)
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[1,2,3])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[1,2,3])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[1,2,3])
    parser.add_argument("--prune_iterations", nargs="+", type=int, default=[2])
    parser.add_argument("--start_checkpoint", type=str, default = None)
    parser.add_argument("--start_pointcloud", type=str, default = None)
    parser.add_argument("--target_num", type=float, default=180_000)
    parser.add_argument("--prune_decay", type=float, default=1)
    parser.add_argument(
        "--prune_type", type=str, default="opacity_scale"
