...
def get_class_id(model_path, name, iteration, gaussians,classifier):
    save_path = os.path.join(model_path, name, "ours_{}".format(iteration), "classid")
    makedirs(save_path, exist_ok=True)
    with torch.no_grad():
        logits3d = classifier(gaussians._objects_dc.permute(2,0,1))
        print(gaussians._objects_dc.permute(2,0,1).size())
        prob_obj3d = torch.softmax(logits3d,dim=0)
        id_obj3d = torch.argmax(prob_obj3d.squeeze(-1).transpose(0,1),dim=1)
        print(id_obj3d.size())
        # directory = os.path.dirname(os.path.join(save_path,"class_ids_log.txt"))
        # if directory and not os.path.exists(directory):
        #     os.makedirs(directory)
        with open(os.path.join(save_path,"class_ids_log.txt"),'w') as f:
            for index,class_id in enumerate(id_obj3d):
                layer_id = index//45000
                f.write(f"{index},{class_id.item()},{layer_id}\n")
        # mask = prob_obj3d[selected_oj_ids, :, :] > removal_thresh
...
def save_class(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree)
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False)
        ...
        get_class_id(dataset.model_path, "train", scene.loaded_iter,gaussians,classifier)
        # if not skip_train:
        #      render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, classifier)
...
if __name__ == "__main__":
    ...
    save_class(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test)